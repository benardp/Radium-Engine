    ...
    if ( m_wireframe )
    {
        ...
        if ( m_lightmanagers[0]->count() > 0 )
        {
            ...
            for ( size_t i = 0; i < m_lightmanagers[0]->count(); ++i )
            {
                ...
                for ( const auto& ro : m_fancyRenderObjects )
                {
                    ro->render( wireframepassParams, renderData, DefaultRenderingPasses::WIREFRAME );
                }
                ...
                for ( const auto& ro : m_transparentRenderObjects)
                {
                    ro->render( wireframepassParams, viewingParameters, DefaultRenderingPasses::LIGHTING_OPAQUE );
                }
#endif
            }
        }
